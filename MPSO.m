...
%%
% Particle Swarm Optimization 粒子群优化算法主函数
function [gBestScore,gBest,cg_curve]=MPSO(N,iter,lb,ub,dim,y,G)
%lb、ub为每一维的上下限；
%y为目标函数或适应度函数，输入为粒子在各维的坐标，输出为适应度值。
%G：与地图、约束或自由栅格相关的矩阵，通常用于约束粒子的可行搜索区域。

% PSO参数初始化
Vmax=ones(1,dim).*(ub-lb).*0.15;           % 速度最大值，每一维的最大速度为上下界差的15%
noP=N;                                     % 粒子数量
% w=0.8;                                     % 惯性权重
w_max=0.9;
w_min=0.4;
...
while l<iter
    ...
    u = randperm(noP, 2);
    for i = 1:noP
        [U, index] = min(pBestScore(u));
        if min(pBestScore(u)) < pBestScore(i)
            Ubest(i, :) = pBest(index, :);
        else
            Ubest(i, :) = pBest(i, :);
        end
    ...
    for i=1:size(pos,1)
        for j=1:size(pos,2)
            % PSO速度更新公式
            %改进点2：速度更新和随机主流学习
            vel(i,j)=w(l)*vel(i,j) + c1*rand()*(Ubest(i,j)-pos(i,j)) + c2*rand()*(mean(pBest(:,j))-pos(i,j)); 
            % 限制速度在[-Vmax, Vmax]范围内
            if(vel(i,j)>Vmax(j))
                vel(i,j)=Vmax(j);
            end
            ...
            for i = 1:noP  %改进点3
                if  exp(fx(i)) / exp(mean(fx)) > rand
                    pos(i,j)=w(l)*pos(i,j)+(1-w(l))*vel(i,j)+gBest(j);
                else
                    pos(i,j)=pos(i,j)+vel(i,j);
                end
            end
            pos(i,:) = Bounds(pos(i,:), lb, ub); % 保证位置在边界内  %新增1
              
        end
    end
    %  %% 捕食者捕食种群中的老弱病残（竞争淘汰机制或末位淘汰机制）
    % 1. 计算所有粒子的适应度
     fx = zeros(N,1);
     for i = 1:N
         fx(i) = y(pos(i,:));  % y为适应度函数，pos为粒子位置
     end
    % 2. 找到全局最差粒子的索引
     [worst, Gw_index] = max(fx);
     Gw = pos(Gw_index, :);   % 最差粒子位置
     % 3. 选出除最差外的所有索引
     z = [1:Gw_index-1, Gw_index+1:N];
     % 4. 随机选两个不同粒子用于差分
     d = randperm(length(z),2);
     j = z(d(1));
     k = z(d(2));
     % 5. 生成新粒子Nb
     Nb = gBest + rand(1, size(pos,2)) .* (pBest(j,:) - pBest(k,:));
     % 6. 限制新粒子在上下界
     Nb = max(lb, min(ub, Nb));
     % 7. 计算新粒子的适应度
     fNb = y(Nb);
